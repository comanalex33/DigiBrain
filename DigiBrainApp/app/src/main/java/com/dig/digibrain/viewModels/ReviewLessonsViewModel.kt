...
class ReviewLessonsViewModel(private val repository: Repository): ViewModel() {

    fun addQuestionToSubjects(authHeader: String, questionId: Long, subjectIds: List<Long>) = liveData(Dispatchers.IO) {
        emit(Resource.loading(data = null))
        try {
            emit(Resource.success(data = repository.addQuestionToSubjects(authHeader, questionId, subjectIds)))
